...
export default function CosmicBackground() {
  ...
  // Responsive device check on mount & resize
  useEffect(() => {
    const checkMobile = () => {
  ...
  // Highly reduced count on mobile (22 stars instead of 65)
  const starsArray = useMemo(() => {
    const starsCount = isMobile ? 22 : 65;
    return Array.from({ length: starsCount }).map((_, i) => ({
      id: i,
      top: `${Math.random() * 100}%`,
      left: `${Math.random() * 100}%`,
      size: `${0.6 + Math.random() * (isMobile ? 1.2 : 1.8)}px`,
  ...
  return (
    <div className="fixed inset-0 -z-20 overflow-hidden bg-bg-dark">
      ...
      {/* Atmospheric planetary rings (Vedic concentric orbit lines) - Skip heavy rotations on mobile */}
      {!isMobile && (
        <>
      ...
      {isMobile ? (
        // Static representation for light, performant mobile rendering
        <div className="absolute top-[10%] right-[-15%] w-[350px] h-[350px] bg-indigo-600/5 rounded-full blur-[100px] pointer-events-none opacity-40" />
      ) : (
        <motion.div
          animate={{
            scale: [1, 1.12, 1],
      ...
      {isMobile ? (
        <div className="absolute bottom-[-15%] left-[-15%] w-[300px] h-[300px] bg-accent/4 rounded-full blur-[90px] pointer-events-none opacity-30" />
      ) : (
        <motion.div
          animate={{
            scale: [1, 1.15, 1],
      ...
      {/* Cosmic Nebula Cloud 3 (Deep Purple Amethyst) - Rendered only on high performance Desktop views */}
      {!isMobile && (
        <motion.div
          animate={{
            scale: [1, 1.08, 1],
