...
async function getAstrologyReading(params: {
  birthDate?: string;
  birthTime?: string;
  birthPlace?: string;
  zodiacSign?: string;
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!apiKey) {
      res.status(400).json({
        error: "नक्षत्र वर्तमान में धुंधले हैं - एपीआई कुंजी गायब है। कृपया Vercel Environment Variables में जाकर GEMINI_API_KEY कॉन्फ़िगर करें और redeploy करें।"
      });
      return;
    }
